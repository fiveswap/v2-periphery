...
contract FiveswapV2Router02 is IFiveswapV2Router02 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = FiveswapV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = FiveswapV2Library.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "FiveswapV2Router: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = FiveswapV2Library.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "FiveswapV2Router: INSUFFICIENT_A_AMOUNT"
                );
    ...
    function addLiquidityPEN(
        ...
        returns (uint amountToken, uint amountPEN, uint liquidity)
    {
        ...
        if (msg.value > amountPEN)
            TransferHelper.safeTransferPEN(msg.sender, msg.value - amountPEN);
    }
    ...
    function removeLiquidityPEN(
        ...
        returns (uint amountToken, uint amountPEN)
    {
    ...
    function removeLiquidityPENWithPermit(
        ...
        (amountToken, amountPEN) = removeLiquidityPEN(
            token,
            liquidity,
            amountTokenMin,
            amountPENMin,
    ...
    function _swap(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? FiveswapV2Library.pairFor(factory, output, path[i + 2])
                : _to;
            IFiveswapV2Pair(FiveswapV2Library.pairFor(factory, input, output))
        ...
        returns (uint[] memory amounts)
    {
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            FiveswapV2Library.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(
            ...
            "FiveswapV2Router: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
    ...
    function swapExactPENForTokens(
        uint amountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ...
    function swapTokensForExactPEN(
        uint amountOut,
        uint amountInMax,
        address[] calldata path,
        address to,
    ...
    {
        require(
            path[path.length - 1] == WPEN,
            "FiveswapV2Router: INVALID_PATH"
        );
        amounts = FiveswapV2Library.getAmountsIn(factory, amountOut, path);
        require(
            amounts[0] <= amountInMax,
    ...
    function swapExactTokensForPEN(
        uint amountIn,
        uint amountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapPENForExactTokens(
        uint amountOut,
        address[] calldata path,
        address to,
        uint deadline
        ...
        returns (uint[] memory amounts)
    {
        ...
        if (msg.value > amounts[0])
            TransferHelper.safeTransferPEN(msg.sender, msg.value - amounts[0]);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = FiveswapV2Library.sortTokens(input, output);
            IFiveswapV2Pair pair = IFiveswapV2Pair(
                FiveswapV2Library.pairFor(factory, input, output)
            );
            uint amountInput;
            uint amountOutput;
            {
                ...
                amountOutput = FiveswapV2Library.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
