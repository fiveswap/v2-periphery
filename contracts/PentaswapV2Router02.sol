...
import '@penta-swap/v2-core/contracts/interfaces/IPentaswapV2Factory.sol';
import '@penta-swap/lib/contracts/libraries/TransferHelper.sol';

import './interfaces/IPentaswapV2Router02.sol';
import './libraries/PentaswapV2Library.sol';
import './libraries/SafeMath.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWPEN.sol';

contract PentaswapV2Router02 is IPentaswapV2Router02 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IPentaswapV2Pair(PentaswapV2Library.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PentaswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = PentaswapV2Library.sortTokens(input, output);
            IPentaswapV2Pair pair = IPentaswapV2Pair(PentaswapV2Library.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = PentaswapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PentaswapV2Library.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'PentaswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
        ...
        ensure(deadline)
    {
        ...
        assert(IWPEN(WPEN).transfer(PentaswapV2Library.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'PentaswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    {
        return PentaswapV2Library.getAmountIn(amountOut, reserveIn, reserveOut);
    }
